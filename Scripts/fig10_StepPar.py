...
def make_figure(alpha="1"):
    ...
    data_dir = "{}/steppar_specs_alpha{}/".format(
        blueshift_util.g_DataDir, alpha)
    ...
    fnames_all = [
        [".spec", "_diskmax_1e18.spec", "_diskmax_3e17.spec",
            "_diskmax_1e17.spec", "_diskmax_3e16.spec"],
        ["_omega0p1.spec", ".spec", "_omega0p3.spec",
            "_omega0p4.spec", "_omega0p5.spec"],
        ["_alpha0.5.spec", "_alpha0.75.spec", "_alpha1.spec",
            "_alpha1.25.spec", "_alpha1.5.spec"],
        [".spec", "_rv_3e18.spec", "_rv_1e18.spec",
            "_rv_3e17.spec", "_rv_1e17.spec"],
        ["_mdotw0.1.spec", "_mdotw0.5.spec",
            "_mdotw1.0.spec", ".spec", "_mdotw10.spec"],
    ...
    for itb, tb in enumerate(par_strings):
        ...
        fnames = [f for f in os.listdir(
            data_dir) if ".spec" in f[-6:] and tb in f]
        ...
        for j, fsuffix in enumerate(fnames):
            ...
            if f == "{}.spec".format(root):
                ...
                path_effects = None
            plt.plot(velocity/1e3, flux, lw=3, label=plot_label,
            ...
            if tb == "omega":
                plt.title(r"${}$ for fixed $\theta_{{\rm min}}$".format(
                    symbols[tb]), fontsize=16)
            else:
                plt.title(r"{}, ${}$".format(
                    titles[tb], symbols[tb]), fontsize=16)

            # plt.grid(ls=":")
            plt.gca().axvline(0, ls="--", alpha=0.5, color="k")
        plt.legend(frameon=True, labelspacing=0.25,
