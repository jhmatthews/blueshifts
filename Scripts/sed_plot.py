...
def plot_seds(ax, sed_names = ["m16_sed", "mean_qso_modified", "qsosed_m9.500_mdot0.468", "qsosed_m9.000_mdot0.150_hispin"], 
                sed_folder = "/Users/matthewsj/winds/python/xdata/seds/model_files/", 
                labels=["Disc$+$PL", "Richards$+$~06", "QSOSED 1", "QSOSED 2"]):

    for i, f in enumerate(sed_names):
        wave, flambda = np.genfromtxt("{}/{}.dat".format(sed_folder,f), unpack=True)
        ...
        ax.plot(nu, nulnu * fcorr, lw=3.5, path_effects = path_effects, label=labels[i])
        plt.loglog()
    ...
    ax.text(1.3e16, 1.7e44, r"C\textsc{iii},C\textsc{iv}", ha="center", fontsize=12)
    ...
    ax.legend(frameon=False, labelspacing=0.25, borderpad=0.25, loc="upper right")


def make_figure():
    jm_util.set_cmap_cycler("magma_r", 6)
    from matplotlib.gridspec import GridSpec

    fig = plt.figure(figsize=(10,4))
    gs1 = GridSpec(1, 2, right=0.58, left=0.065, bottom=0.15, top=0.98, wspace=0.05)
    ...
    labels = ["Disc$+$PL", "Richards$+$~06", "QSOSED 1", "QSOSED 2", "Anisotropic Disc"]
    mod_name = ["e)", "n)"]
    ...
    for i,fnames in enumerate(fname_list):
        ...
        for j,f in enumerate(fnames):
            ...
            if f == "run47.spec" or f == "run128.spec":
                ...
                path_effects = None
            ax.plot(velocity/1e3, flux/4, lw=lw, path_effects=path_effects, label=label_use, c=color, ls=ls, zorder=zord)
            ax.set_xlim(-8,8)
            ax.set_ylim(1, 20)
            if i == 0:
        ...
        ax.set_xlabel("Velocity~($10^3$~km~s$^{-1}$)", fontsize=18, labelpad=-1)
    ...
    blueshift_util.save_paper_figure("fig12.pdf")
    print ("All Done.")
