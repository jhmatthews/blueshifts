...
def plot_full_spectra(ax, files, colors, args, folder):
    ...
    # "C \textsc{ivi}"
    labels = [r"O\textsc{i}",r"C\textsc{ii}", r"Si\textsc{iv}$+$O\textsc{iv}]",r"He \textsc{ii}",r"O \textsc{iii}]",r"Al \textsc{iii}",r"C \textsc{iii}]"]
    y = [0.58,0.77,0.58,0.58,0.77,0.77,0.58]
    for i,l in enumerate(lines):
        ax.text(l - 10, y[i], labels[i], fontsize=16, horizontalalignment="left")

    
    ax.vlines([lines],0.65,0.75, color="k")
    ax.text(1550,3.8,r"C\textsc{iv}~1550\AA", fontsize=16, color="k", horizontalalignment="center")
...
for itb, tb in enumerate(["thmin45"]):
    ...
    #if 
    fnames = [f for f in os.listdir(data_dir) if ".spec" in f[-6:] and ((tb in f and "thmin45_rv1e19_vinf1p0_mdotw5p0" in f) or "run128_aniso" in f) and "tau" not in f]
    ...
    for f in fnames:
        ...
        for iangle, angle in enumerate(angles):
            ...
            plt.plot(w, fl, alpha=1, c=mappable.to_rgba(angles[iangle]), zorder=5, lw=2.5)
        ...
        cbar.set_label(r"$\log[{\rm C}~\textsc{iv}~{\rm fraction}]$", labelpad=-2)
        #cbar.set_clim(-4,0)
       #plt.loglog()

        xx = np.linspace(0,1e20,1000)
        for iangle, angle in enumerate(angles):
            ax.plot(xx, xx*np.tan(np.radians(90-angle)), c=mappable.to_rgba(angles[iangle]), alpha=1, ls="-", lw=1.5)
