...
def make_figure():
    ...
    for f in fnames:
        ...
        if iplot == 0:
            ...
            plt.xlabel(r"Velocity~$({\rm km~s}^{-1})]$", fontsize=18, labelpad=-2)
        else:
        ...
        if iplot == 0:
            plt.xlabel(r"$\log[v_{\phi}~({\rm km~s}^{-1})]$", fontsize=18, labelpad=-2)
    ...
    cbar = plt.colorbar(cax = ax_cbar, shrink=0.5, fraction=0.1, mappable = mappable, orientation="horizontal", extend="both")
    ...
    for alpha in alphas:
        ...
        v, vinf, vrot = vel_law(l, R_v = R_v, r0=3e16, v0=1e6, fesc = 1.0, alpha=alpha, theta=45.0)
        ax2.plot(l/R_v, v/vinf, path_effects=path_effects, lw=3, label=r"$v_l/v_\infty, \alpha={}$".format(alpha))
